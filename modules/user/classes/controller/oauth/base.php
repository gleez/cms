...
abstract class Controller_OAuth_Base extends Template {
	...
	public function before()
	{
		...
		if( Auth::instance()->logged_in() ) $this->request->redirect('user/profile');
	...
	public function action_index()
	{
		...
		{
			$url = $this->route->uri(array('controller' => $this->provider->name, 'action' => 'login'));
	...
	public function action_login()
	{
		try
		{
			...
			$callback = URL::site($this->route->uri(
					array('controller' => $this->provider->name, 'action' => 'callback')), 'http');
		...
		try
		{
			...
			if ($this->token)
			{
				...
				$this->request->redirect( $this->route->uri(
					array('controller' => $this->provider->name, 'action' => 'complete')) );
			}
			...
			// Redirect to the provider's index page
			$this->request->redirect( $this->route->uri( array('controller' => $this->provider->name)) );
		}
		catch( Exception $e)
		{
			Kohana::$log->add(LOG::ERROR, (string) $e);
	...
	protected function sso_signup( $data, $user = false )
	{
		...
		try
		{
			...
			if( $user instanceof Model_User )
			{
				...
				if( $user->loaded() AND Auth::instance()->logged_in() == false )
				{
					// Log in as this user
					Auth::instance()->force_login($user);
				
					Message::success(__('Welcome back, :nick logged in via (:provider).',
						array(':nick' => $user->nick, ':provider' =>  $this->provider->name )) );
				}
			}
			else
			{
