...
class Kohana_Config {
	...
	public function load($group)
	{
		...
		//get from cache
		if ( $group != 'cache' AND $group != 'database' AND $data = Gleez::cache("config-{$group}") )
		{
			$this->_groups[$group] = new Config_Group($this, $group, $data);
			unset($data);
		}
		...
		//save to cache
		if( $group !== 'cache' AND $group !== 'database')
		{
			Gleez::cache("config-{$group}", $config, DATE::HOUR);
		}
