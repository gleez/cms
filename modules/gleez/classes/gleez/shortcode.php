...
abstract class Gleez_Shortcode {
	...
	/**
	 * @var  bool Indicates whether shortcodes are cached
	 */
	...
	/**
	 * Retrive(s) all named shortcodes.
	 *
	...
	{
		return Shortcode::$_tags;
	}

	/**
	 * Saves or loads the Shortcode cache. If your Shortcodes will remain the same for
	 * a long period of time, use this to reload the Shortcodes from the cache
	 ...
	 *     }
	 *
	 * @param   boolean $save   cache the current Shortcodes
	 * @param   boolean $append append, rather than replace, cached Shortcodes when loading
	 * @return  void    when saving Shortcodes
	 * @return  boolean when loading Shortcodes
	 * @uses    Kohana::cache
	 */
	public static function cache($save = FALSE, $append = FALSE)
	{
		if ($save === TRUE)
		{
			...
			Kohana::cache('Shortcode::cache()', Shortcode::$_tags);
		}
		else
		{
			if ($tags = Kohana::cache('Shortcode::cache()'))
			{
				if ($append)
				{
					// Append cached Shortcodes
					Shortcode::$_tags += $tags;
				}
				else
				{
				...
				return Shortcode::$_cache = TRUE;
			}
			else
			{
				// Shortcodes were not cached
				return Shortcode::$_cache = FALSE;
			}
		}
	}
