...
class Controller_Admin_Menu_Item extends Controller_Admin {
	...
	public function action_add()
	{
		...
		$view = View::factory('admin/menu/item/form')->bind('menu', $menu)->bind('post', $post)->bind('errors', $errors);
			...
			try
			{
				...
				if ( ! $this->_internal)
					$this->request->redirect(
						Route::get('admin/menu/item')->uri( array( 'action' => 'list', 'id' => $menu->id ) )
					);
	...
	public function action_edit()
	{
		...
		$view  = View::factory('admin/menu/item/form')->bind('menu', $menu)->bind('post', $menu)->bind('errors', $errors);
		...
		if ($this->valid_post('menu-item'))
		{
			try
			{
				...
				if ( ! $this->_internal)
					$this->request->redirect(
						Route::get('admin/menu/item')->uri( array( 'action' => 'list', 'id' => $menu->scp ) )
					);
				...
				Cache::instance('menus')->delete_all();
                        ...
                        catch (Exception $e)
                        {
							...
							array(':id' => $menu->id, ':message' => $e->getMessage()));
                                ...
                                if ( ! $this->_internal)
                                        $this->request->redirect(
						Route::get('admin/menu')->uri(array('action' =>'list', 'id' => $menu->scp )) );
                        }
                }
	...
	public function action_confirm()
        {
				...
				$this->request->redirect(
                                        Route::get('admin/menu/item')->uri( array( 'action'=>'list', 'id' => $id ) )
                                );
                        }
                        ...
                        try
                        {
                                ...
                                {
                                        DB::update('menus')
						->set( array('pid' => $node['pid'], 'active' => $node['active'],
