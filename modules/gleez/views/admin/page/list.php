			...
			<tbody>
				<?php foreach ($posts as $post):  ?>
					<tr id="post-row-<?php echo $post->id ?>" class="<?php echo Text::alternate("odd", "even"); ?>">
						...
						<td>
							<?php echo HTML::anchor($post->url, $post->title, array('class'=>'action-view')) ?>
						</td>
						<td>
							<?php echo HTML::anchor(Route::get('user')->uri(array('action' => 'profile', 'id' => $post->author)), $post->user->nick, array()) ?>
						</td>
						<td class="status">
							<span class="label label-<?php echo $post->status; ?>"><?php echo $post->status; ?></span>
						</td>
