...
class Controller_Admin_Widget extends Controller_Admin {
	...
	public function action_index()
		...
		$theme_name = Kohana::$config->load('site.theme', 'anytime');
		...
		if( isset($theme->regions) AND !empty($theme->regions))
		{
			$widget_regions = $theme->regions;
                ...
                $widget_regions  = $widget_regions + array(self::$WIDGET_REGION_NONE => self::$WIDGET_REGION_NONE);
		...
		Assets::js('widgets', 'media/js/widgets.js', array('jquery'), FALSE, array('weight' => 5));
			...
			Assets::tabledrag('widgets', 'order', 'sibling', 'widget-weight', 'widget-weight-' . $region);
                ...
                if ( $this->valid_post('widget-list') )
                {
                        foreach ($_POST['widgets'] as $widget)
                        {
                                ...
                                DB::update('widgets')->set( array('status'=> $widget['status'],
			...
			if ( ! $this->_internal) $this->request->redirect( Route::get('admin/widget')->uri() );
                }
                ...
                if ( $this->valid_post('widget') )
		{
			...
			{
				$widget->save();
				if( isset($_POST['widget']) ) unset( $_POST['widget'], $_POST['_token'], $_POST['_action'] );
		...
		if( $static === FALSE ) $this->request->redirect( Route::get('admin/widget')->uri() );
		$handler = Widget::factory($widget->name, $widget);
		...
		// If deletion is confirmed
				...
				Message::error('An error occured deleting widget, :post.',array(':post' => $widget->title));
                        }
