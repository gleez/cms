...
class Gleez_Core {
  ...
  public static function ready()
  {
    ...
    if (! file_exists(APPPATH.'config/database.php'))
    {
      ...
      Route::set('install/media', 'setup/media(/<file>)',
        array(
        'file' => '.+'
        ))
        ->defaults(array(
  ...
  {
    // Eenable cache only in production environment
  ...
  public static function maintenance_mode()
  {
    ...
    if ($maintenance_mode
          AND ($request instanceof Request)
          AND ($request->controller() != 'user' AND $request->action() != 'login')
          AND !ACL::check('administer site')
          AND $request->controller() != 'media')
    {
